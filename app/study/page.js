...
import { shuffle, speak, startRecorder } from '@/lib/utils'
...
export default function Study() {
  ...
  // Recording state
  const [recState, setRecState] = useState(null) // { stop, blob, url, audio, stopAll }
  ...
  async function handleStartRecording() {
    try {
      setIsRecording(true)
      const rec = await startRecorder() // returns { stop, stream } now; full data after stop()
      setRecState(rec)
    } catch (e) {
  ...
  async function handleStopAndPlay() {
    try {
      if (!recState?.stop) return
      await recState.stop() // triggers onstop in startRecorder
      // small delay lets the promise resolve with blob/url/audio on recState
      setTimeout(() => {
        setIsRecording(false)
        if (recState?.audio) {
          recState.audio.currentTime = 0
          recState.audio.play().catch(() => {})
        }
        recState?.stopAll?.()
      }, 50)
    } catch (e) {
  ...
  return (
    <main className="grid" style={{ gap: 24 }}>
      <header className="grid" style={{ gap: 8 }}>
        <h1>Study</h1>
        <p className="helper">
          Hear the word, then type it. Toggle tiles if your child prefers to build the word by tapping letters.
        </p>
      </header>
          ...
          {current ? (
            <section className="card grid" style={{ gap: 16 }}>
              ...
              <div className="row">
                ...
                {isRecording && <button onClick={handleStopAndPlay}>Stop & Play</button>}
              </div>
                  ...
                  <div className="tile-row">
                    {letters.map((ch, i) => (
                      <div
                        ...
                        onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { setAnswer(a => a + ch) } }}
                      >
                        {ch}
                      </div>
                    ))}
